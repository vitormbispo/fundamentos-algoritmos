...
def main():
    ...
    medalhas:list[Medalha] = criar_lista_medalhas(tabela) # - Transforma a matriz no tipo estruturado
    paises:list[Pais] = criar_lista_paises(medalhas) # - Separa os dados das medalhas para cada país
...
def criar_lista_medalhas(medalhas:list[list:str]) -> list[Medalha]:
    '''
    Transforma as informações de medalhas na matriz de strings 'medalhas'em
    uma lista de estruturas do tipo 'Medalha'.

    Exemplos de conversão em tipo estruturado:
    ['1','W',"BRA"]
    ...
    medalhas_estruturadas:list[Medalha] = []
    for medalha in medalhas:
        ...
        medalhas_estruturadas.append(nova_medalha)
    
    return(medalhas_estruturadas)

def criar_lista_paises(medalhas:list[Medalha]) -> list[Pais]:
    '''
    Cria uma lista do tipo estruturado 'Pais' com todos os países 
    contemplados com medalhas na lista 'medalhas'

    Exemplo de conversão para 'Pais':
    ...
    for medalha in medalhas:
        i+=1
        if indice_do_pais(medalha.pais,paises) == -1: # - País não existe na lista, adicionando...
            novo_pais:Pais = Pais([medalha],[0,0,0],medalha.pais,1)
            novo_pais.contagem_medalhas[medalha.tipo.value-1] = 1 # - Código da medalha -1 = índice na lista de contagem de medalhas

            paises.append(novo_pais)

        else: # - Pais existe. Adicionando medalha...
            pais:Pais = paises[indice_do_pais(medalha.pais,paises)]

            pais.medalhas.append(medalha)
            pais.contagem_medalhas[medalha.tipo.value-1] = pais.contagem_medalhas[medalha.tipo.value-1] + 1 # - Código da medalha -1 = índice na lista de contagem de medalhas
...
| QUADRO DE MEDALHAS | OLIMPÍADAS DE TOKYO 2020 |
_________________________________________________
| País | Ouro | Prata | Bronze | Total | Clas.  |
‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾
'''
...
def possui_apenas_um_genero(medalhas:list[Medalha],i:int = 0,genero = Genero.M) -> bool:
    ...
    desconsiderando países que só venceram em categorias mistas.
